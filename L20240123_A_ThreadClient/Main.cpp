...
unsigned WINAPI SendThread(void* Arg)
{
	...
	//3way syn+ack syn Flodding//keep_alive_fine_out
...
int main()
{
	...
	ServerSockAddr.sin_addr.s_addr = inet_addr("127.0.0.1");//192.168.3.118
	ServerSockAddr.sin_port = htons(22222);

	connect(ServerSocket, (SOCKADDR*)&ServerSockAddr, sizeof(ServerSockAddr));//모바일은 그렇게 노놉 UUID식별자 만들어야됨JWT
